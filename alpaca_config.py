"""
app.py – Professional AI Trading Dashboard (Alpaca-Only, GPU-Ready, Plotly Charts)
"""
...
def get_historical_data(symbol, days=60):
    try:
        ...
        if is_crypto:
            bars = api.get_crypto_bars(bars_symbol, "1Day",
                                       start=start.isoformat(),
                                       end=end.isoformat()).df
        else:
            bars = api.get_bars(bars_symbol, "1Day",
                                start=start.isoformat(),
                                end=end.isoformat()).df
...
# ------------------------------
# Sidebar – Ticker management & controls
# ------------------------------
with st.sidebar:
    st.title("🤖 AI Trading Cockpit")

    # Add ticker
    st.subheader("➕ Add Ticker")
    new_ticker = st.text_input("Symbol (e.g., AAPL, BTC/USDT)")
    if st.button("Add"):
        clean = new_ticker.strip().upper()
        if clean and clean not in st.session_state.monitored_tickers:
            if is_tradeable(clean):
                st.session_state.monitored_tickers.append(clean)
                st.session_state.per_ticker_enabled[clean] = False  # safety off
                st.success(f"✅ {clean} added (Auto‑trading OFF)")
            else:
                st.error("❌ Asset not found on Alpaca")
        elif clean:
    ...
    if st.session_state.monitored_tickers:
        for ticker in st.session_state.monitored_tickers:
            ...
            if col_remove.button("❌", key=f"remove_{ticker}", help=f"Remove {ticker}"):
        ...
        st.info("No tickers added yet. Use the text box above.")
    ...
    if st.button("🛑 EMERGENCY STOP", type="primary", use_container_width=True):
        ...
        st.success("All trading halted. Per‑ticker auto toggles disabled.")

# ------------------------------
# Main Dashboard
# ------------------------------
st.title("🚀 Professional AI Trading Dashboard")

if not st.session_state.monitored_tickers:
    st.info("👈 Add tickers in the sidebar to begin.")
else:
    st.subheader("📡 Live Signals & Price Charts")
    for ticker in st.session_state.monitored_tickers:
        ...
        if bars is None:
            st.warning(f"**{ticker}** – No data available")
            continue
        ...
        # ---------- NEW: 5‑day CANDLESTICK CHART ----------
        if bars is not None and len(bars) >= 5:
            recent = bars.tail(5).copy()
            fig = go.Figure(data=[go.Candlestick(
                ...
                low=recent['low'],
                close=recent['close'],
                increasing_line_color='#26a69a',  # bullish green
                decreasing_line_color='#ef5350',  # bearish red
            )])
            fig.update_layout(
                title=f"{ticker} – Last 5 Days",
                xaxis_title="Day",
                yaxis_title="Price ($)",
                template="plotly_dark",
                height=300,
                margin=dict(l=20, r=20, t=40, b=20),
                xaxis_rangeslider_visible=False,   # No rangeslider
            ...
            st.caption("Not enough data for candlestick chart.")

# ------------------------------
# Live Account Equity with Delta
# ------------------------------
...
if equity is not None:
    ...
    <div class="equity-metric">
        <div>
            ...
            <div style="font-size:28px; font-weight:700; color:#000;">${equity:,.2f}</div>
        </div>
    ...
    if not st.session_state.equity_history or st.session_state.equity_history[-1]["Equity"] != equity:
        st.session_state.equity_history.append({"Time": now, "Equity": equity})
        if len(st.session_state.equity_history) > 100:
            st.session_state.equity_history.pop(0)

    # ---------- NEW: PROFESSIONAL EQUITY LINE CHART ----------
    ...
    st.warning("Could not fetch account equity.")
...
if positions:
    ...
    st.dataframe(
        df_pos.style.format({
            ...
            "Market Value": "${:,.2f}",
            "Unrealized P&L": "${:,.2f}"
        }).applymap(color_pl, subset=['Unrealized P&L']),
